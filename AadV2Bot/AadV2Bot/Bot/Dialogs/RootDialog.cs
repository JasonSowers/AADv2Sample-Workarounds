    ...
    public class RootDialog : IDialog<object>
    {
        ...
        private async Task MessageReceivedAsync(IDialogContext context, IAwaitable<object> result)
        {
            ...
            if (message.ToLowerInvariant().Equals("recents"))
            {
                ...
                context.Call(CreateSignInDialog(), ListRecentMail);
            }
            else if (message.ToLowerInvariant().StartsWith("send"))
            {
                ...
                {
                    context.Call(CreateSignInDialog(), async (IDialogContext ctx, IAwaitable<GetTokenResponse> tokenResponse) => {
            ...
            else if (message.ToLowerInvariant().Equals("me"))
            {
                if (activity.ChannelId == ChannelIds.Sms 
                    || activity.ChannelId == ChannelIds.Slack 
                    || activity.ChannelId == ChannelIds.Kik
                    || activity.ChannelId == ChannelIds.Telegram
                    || activity.ChannelId == ChannelIds.Groupme
                    || activity.ChannelId== ChannelIds.Msteams)
                {
